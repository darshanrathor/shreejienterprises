import React from 'react'

const Section2 = () => {
  return (
    <div>
        
       <div className='md:px-38 md:mt-10 md:p-[40px]'>

{/*
<div className='flex flex-col text-center items-center mb-20 mt-14   ' >

  <div className=' text-center font-serif uppercase mb-7 text-4xl font-medium leading-tight '> 
  <h1 className='text-[#010A10]' >OUR <span className='text-[#C10206]'>SERVICES </span> </h1>
  </div>

  <div className=" max-w-[600px] text-center items-center justify-center ">
    <h3>
    THE SIZE OF OUR TEAM AND OUR PROTRACTED EXPERTISE  ALLOW US TO ACCOMPLISH ABSOLUTELY ANY TYPE OF A BUILDING, REGARDLESS OF ITS HEIGHT AND/OR COMPLEXITY!
    </h3>
  </div>

  </div>*/}

<div className='flex flex-col text-center items-center mb-10 mt-  ' >
<div className=" max-w-[600px] text-center items-center justify-center ">
    <h3>
    THE SIZE OF OUR TEAM AND OUR PROTRACTED EXPERTISE ALLOW US TO ACCOMPLISH ABSOLUTELY ANY TYPE OF A BUILDING, REGARDLESS OF ITS HEIGHT AND/OR COMPLEXITY!
    </h3>
  </div>

</div>

<div class="grid text-[#010A10] grid-cols-1 m-7 gap-9  grid-rows-1 md:grid-cols-2">

  {/* 1st */}
<div
class="mr-4 flex  bg-white ">
<div className='flex '>
  
<svg fill="#000000" version="1.1" id="Capa_1" 
	 className='w-[300px] h-[70px] transition-all  hover:fill-white hover:bg-red-700 mt-6 fill-red-700 bg-white rounded-full' viewBox="0 0 459.428 459.428"
	 >
<g>
	<path d="M349.792,157.708l-19.856,19.856c9.316,17.136,14.62,36.652,14.62,57.459c0,66.232-53.924,120.156-120.156,120.156
		s-120.156-53.924-120.156-120.156c0-66.232,53.924-120.156,120.156-120.156c20.808,0,40.324,5.304,57.459,14.62l19.856-19.856
		c-22.508-13.94-48.96-21.964-77.316-21.964c-81.26,0-147.356,66.096-147.356,147.356S143.14,382.38,224.4,382.38
		s147.356-66.096,147.356-147.355C371.756,206.669,363.731,180.217,349.792,157.708z M294.644,212.925l-23.868,23.801
		c-0.884,24.887-21.283,44.742-46.375,44.742c-25.636,0-46.444-20.807-46.444-46.443c0-25.092,19.856-45.492,44.744-46.375
		l23.868-23.8c-7.004-2.244-14.416-3.468-22.167-3.468c-40.596,0-73.644,33.048-73.644,73.644s33.048,73.645,73.644,73.645
		s73.644-33.049,73.644-73.645C298.044,227.34,296.888,219.861,294.644,212.925z M416.771,119.629l-19.855,19.856
		c15.708,28.288,24.684,60.86,24.684,95.54c0,108.732-88.468,197.201-197.2,197.201S27.2,343.757,27.2,235.024
		c0-108.732,88.468-197.2,197.2-197.2c34.68,0,67.251,8.976,95.54,24.684l19.856-19.856C306.067,22.321,266.56,10.625,224.4,10.625
		C100.64,10.625,0,111.265,0,235.024s100.64,224.4,224.4,224.4s224.4-100.641,224.4-224.4
		C448.8,192.865,437.104,153.357,416.771,119.629z M387.301,120.207l-25.963-2.883L233.431,245.226
		c-5.311,5.311-13.92,5.311-19.231,0c-5.311-5.312-5.311-13.92,0-19.231L342.101,98.093l-2.883-25.962l72.128-72.128l9.615,38.468
		l38.467,9.615L387.301,120.207z"/>
</g>
</svg>
  
<div class="p-6">
<h5
  class="mb-2 text-xl font-medium leading-tight text-[#010A10] ">
  Mission
</h5>
<p class="mb-4 text-base text-neutral-600 dark:text-neutral-200">
To provide quality workmanship and customer service and maintain the
highest level of professionalism, honesty and fairness in our relationship with
our customers, employees and vendors. To grow by continually providing
useful and significant products and services.
</p>
</div>

</div>
</div>


{/* 2nd */}

<div
class="mr-4 flex rounded-lg bg-white ">
<div className='flex  top-0'>

<svg viewBox="0 0 24 24" fill="none" className='w-[145px] h-[70px] transition-all  hover:fill-white
	  hover:bg-red-700 mt-6  bg-white border-red-700 border-2 rounded-full'>
<g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" 
stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> 
<path d="M9 4.45962C9.91153 4.16968 10.9104 4 12 4C16.1819 4 19.028 6.49956 20.7251 
8.70433C21.575 9.80853 22 10.3606 22 12C22 13.6394 21.575 14.1915 20.7251 15.2957C19.028
 17.5004 16.1819 20 12 20C7.81811 20 4.97196 17.5004 3.27489 15.2957C2.42496 14.1915 2
  13.6394 2 12C2 10.3606 2.42496 9.80853 3.27489 8.70433C3.75612 8.07914 4.32973 7.43025 
  5 6.82137" stroke="#c60c0c" stroke-width="1.5" stroke-linecap="round"></path>
   <path d="M15 12C15 13.6569 13.6569 15 12 15C10.3431 15 9 13.6569 9 12C9 10.3431 10.3431 
   9 12 9C13.6569 9 15 10.3431 15 12Z" stroke="#c60c0c" stroke-width="1.5"></path> </g></svg>
<div class="p-6">
<h5
  class="mb-2 text-xl font-medium leading-tight text-neutral-800 dark:text-neutral-50">
  Vision
</h5>
<p class="mb-4 text-base text-neutral-600 dark:text-neutral-200">
To be the preferred contractor of choice. A company that our clients want to
work with and our employees are proud to work for.
</p>
</div>

</div>
</div>

{/* 3nd */}


<div
class="mr-4 flex  bg-white ">
<div className='flex  top-0'>
  
<svg fill="#000000"    version="1.1" id="Capa_1" 
	 viewBox="0 0 407.834 407.834"  className='w-[300px] h-[70px] transition-all  hover:fill-white
	  hover:bg-red-700 mt-6 fill-red-700 bg-white border-red-700 border-2 rounded-full' >
<g>
	<path d="M66,217.709v-35c0-1.657-1.343-3-3-3H3c-1.657,0-3,1.343-3,3v35c0,1.657,1.343,3,3,3h60
		C64.657,220.709,66,219.366,66,217.709z"/>
	<path d="M132.666,151.708h60c1.657,0,3-1.343,3-3v-35c0-1.657-1.343-3-3-3h-60c-1.657,0-3,1.343-3,3v35
		C129.666,150.365,131.01,151.708,132.666,151.708z"/>
	<path d="M292,273.375h-60c-1.657,0-3,1.343-3,3v35c0,1.657,1.343,3,3,3h60c1.657,0,3-1.343,3-3v-35
		C295,274.718,293.657,273.375,292,273.375z"/>
	<path d="M404.834,93.459h-60c-1.657,0-3,1.343-3,3v35c0,1.657,1.343,3,3,3h60c1.657,0,3-1.343,3-3v-35
		C407.834,94.802,406.491,93.459,404.834,93.459z"/>
	<path d="M371.499,139.15c-11.045,0-20,8.954-20,20c0,0.64,0.037,1.271,0.096,1.896l-88.943,66.643
		c-2.744-1.417-5.851-2.23-9.152-2.23c-3.186,0-6.186,0.764-8.859,2.089l-62.014-47.962c0.01-0.266,0.04-0.526,0.04-0.794
		c0-11.045-8.955-20-20-20c-11.045,0-20,8.955-20,20c0,0.267,0.03,0.526,0.04,0.79l-58.854,44.996
		c-2.367-0.986-4.962-1.536-7.686-1.536c-11.045,0-20,8.955-20,20c0,11.046,8.955,20,20,20s20-8.954,20-20
		c0-0.871-0.075-1.722-0.183-2.564l57.477-43.943c2.757,1.433,5.883,2.256,9.206,2.256c3.317,0,6.439-0.82,9.193-2.25l61.701,47.72
		c-0.024,0.398-0.061,0.792-0.061,1.197c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20c0-0.295-0.031-0.582-0.044-0.874
		l89.753-67.25c2.528,1.155,5.329,1.815,8.29,1.815c11.045,0,20-8.955,20-20C391.499,148.105,382.545,139.15,371.499,139.15z"/>
</g>
</svg>
  
<div class="p-6">
<h5
  class="mb-2 text-xl font-medium leading-tight text-neutral-800 dark:text-neutral-50">
  Values
</h5>
<p class="mb-4 text-base text-neutral-600 dark:text-neutral-200">
We feel that our services are delivered to the highest standards at all times and
we continually monitor the progress of our projects. Our aim is to provide the
best possible service within the utility & construction industry whilst
maintaining the highest standard of customer satisfaction we can achieve.
</p>
</div>

</div>
</div>

{/* 4 Th */}


<div
class="mr-4 flex rounded-lg bg-white  dark:bg-neutral-700">
<div className='flex  top-0'>
  
<svg  viewBox="0 0 24 24"  className='w-[300px] h-[70px] transition-all fill-white    
	  hover:bg-red-700 mt-6 bg-white border-red-800 border-2 rounded-full'>
<path d="M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5V7H9V5Z" stroke="#A50113" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M9 5H7C5.89543 5 5 5.89543 5 7V19C5 20.1046 5.89543 21 7 21H10M15 5H17C18.1046 5 19 5.89543 19 7V12" stroke="#A50113" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M13 19L15 21L20 16" stroke="#A50113" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</svg> 
<div class="p-6">
<h5
  class="mb-2 text-xl font-medium leading-tight text-neutral-800 dark:text-neutral-50">
  Standards
</h5>
<p class="mb-4 text-base text-neutral-600 dark:text-neutral-200">
Uncompromising integrity as our foundation, we will inspire, engage, create &
deliver. Using our resources and human talent to their best advantage. 
Educating the employees with the latest trends of the business. Making
innovations so that we walk hand in hand with the current changes. 
</p>
</div>

</div>
</div>










</div>


















</div>








    </div>
  )
}

export default Section2